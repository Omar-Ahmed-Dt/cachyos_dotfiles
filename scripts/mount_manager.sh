...
# Requires: cryptsetup, lsblk, udisks2 (udisksctl), fuzzel, coreutils, mount/umount
#
# Usage examples:
#   ./storage_manager.sh open           # Select a crypto_LUKS device via fuzzel -> unlock & mount to /mnt/myssd
#   ./storage_manager.sh close          # Unmount & lock last opened encrypted device -> power off its base disk
#   ./storage_manager.sh mount          # Select plain device via fuzzel -> mount to /mnt/usb
...
open_encrypted() {
    ...
    if command -v fuzzel >/dev/null 2>&1; then
        ...
        local input
        input="$(printf "%s\n" "$proposed" | fuzzel --dmenu -p "Mapper name:")" || true
...
# 1. Plain Devices:
  mount               Mount a plain device to $PLAIN_MOUNTPOINT (fuzzel to select)
  unmount-poweroff    Unmount plain device and power off its base disk

# 2. Encrypted Devices:
  open                Unlock a LUKS (crypto_LUKS) device, mount to $CRYPT_MOUNTPOINT (fuzzel to select)
...
Env vars you may tweak:
  ...
  DMENU_CMD="$DMENU_CMD"  (default: fuzzel --dmenu -p)
